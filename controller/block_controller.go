...
// @Router       					  /api/block/get/{userid} [get]
...
type Resp_FindUserAndCreateBlock struct {
	Block model.Block
	Objs  []model.Obj
}
...
// @Router       					/api/block/data/{userid} [get]
func FindUserAndCreateBlock(ctx *gin.Context) {
	...
	idToString := strconv.Itoa(result.Id)

	objs, _ := model.ObjSchema.GetObjsByBlockId(configs.ConnectDB(), idToString)
	// Return Result To FE
	// 4. 가지고 있다면 블록을 리턴한다.
	ctx.JSON(http.StatusOK, gin.H{
		"block": result,
